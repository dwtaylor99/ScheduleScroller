    ...
    def draw_screen(self):
        ...
        if self.trivia_question is not None:
            if self.game_type == GameType.STINGER:
                gradient.rect_gradient_h(alt_screen, DK_GRAY, BLACK, pygame.Rect(0, 0, WIDTH, HEIGHT))
                alt_screen.blit(TXT_STINGER, ((WIDTH - TXT_STINGER.get_width()) // 2, 10))
                alt_screen.blit(self.stinger_img, ((WIDTH - self.stinger_img.get_width()) // 2,
                                                   (HEIGHT - self.stinger_img.get_height()) // 2))

            elif self.game_type == GameType.TRIVIA:
                gradient.rect_gradient_h(self.screen, DK_GRAY, BLACK, pygame.Rect(0, 0, WIDTH, HEIGHT))
                ques = wrap_text(self.trivia_question.question).strip().split("\n")
                text_height = (HEIGHT - (36 * len(ques))) // 2
                ...
                gradient.rect_gradient_h(alt_screen, DK_GRAY, BLACK, pygame.Rect(0, 0, WIDTH, HEIGHT))
                alt_screen.blit(TXT_EMOJI, ((WIDTH - TXT_EMOJI.get_width()) // 2, HEIGHT // 2 - 70))
                txt = FONT_EMOJI_LG.render(self.trivia_question.question.split(":")[1], True, WHITE)
                alt_screen.blit(txt, ((WIDTH - txt.get_width()) // 2, HEIGHT // 2))

        else:
            alt_screen.blit(TXT_NEXT, ((WIDTH - TXT_NEXT.get_width()) // 2, (HEIGHT - TXT_NEXT.get_height()) // 2))
